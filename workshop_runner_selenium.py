...
def setForm(driver, values):
    validateInput(form["input"], values)

    setFormText(driver, values[0])
    setFormSelect(driver, values[1])

def validateInput(reference, input):
...
@pytest.mark.parametrize(ecfeed.test_header(method, feedback=True), ecfeed.nwise(method=method, feedback=True, constraints='NONE'))
def test_method_random(Country, Name, Address, Product, Color, Size, Quantity, Payment, Delivery, Phone, Email, test_id):

    timeStart = time.time()

    try:
        setForm(driver, [[Name, Address, Quantity, Phone, Email], [Country, Product, Color, Size, Payment, Delivery]])
    ...
    assert response[0] == " Request accepted", ecfeed.feedback(test_id, False, duration=timeTotal, comment=''.join(e + ' ' for e in response), custom={'Error type': 'Output'})
